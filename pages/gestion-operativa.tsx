      ...
      <div style={{...styles.publishCard,maxWidth:1980,margin:'0 auto 18px',borderColor:'rgba(16,185,129,.34)',background:'radial-gradient(circle at 0% 0%,rgba(16,185,129,.12),transparent 34%), linear-gradient(180deg,rgba(4,24,31,.95),rgba(4,14,29,.90))'}}>
        <div style={{display:'flex',justifyContent:'space-between',gap:18,alignItems:'flex-start',flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <span style={{fontSize:14,fontWeight:950,color:chatSimulation?.enabled?'#86efac':'#94a3b8'}}>{chatSimulation?.enabled?'ACTIVA':'INACTIVA'}</span>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:10,marginTop:16}}>
          <div><div style={{...styles.fieldLabel,fontSize:13}}>Frecuencia mínima (seg)</div><div style={styles.inputShell}><input type="number" min={5} value={chatSimMinInterval} onChange={e=>setChatSimMinInterval(e.target.value)} style={{...styles.inputInside,paddingLeft:16}} /></div></div>
          <div><div style={{...styles.fieldLabel,fontSize:13}}>Frecuencia máxima (seg)</div><div style={styles.inputShell}><input type="number" min={5} value={chatSimMaxInterval} onChange={e=>setChatSimMaxInterval(e.target.value)} style={{...styles.inputInside,paddingLeft:16}} /></div></div>
          <div><div style={{...styles.fieldLabel,fontSize:13}}>Máximo por sesión</div><div style={styles.inputShell}><input type="number" min={1} max={1000} value={chatSimMaxMessages} onChange={e=>setChatSimMaxMessages(e.target.value)} style={{...styles.inputInside,paddingLeft:16}} /></div></div>
          <div><div style={{...styles.fieldLabel,fontSize:13}}>Mensajes ganadores (%)</div><div style={styles.inputShell}><input type="number" min={0} max={100} value={chatSimWinnerPct} onChange={e=>setChatSimWinnerPct(e.target.value)} style={{...styles.inputInside,paddingLeft:16}} /></div><div style={{...styles.fieldHelp,fontSize:12,fontSize:12}}>Ej: 85 = aproximadamente 85% positivos y 15% negativos.</div></div>
          <div><div style={{...styles.fieldLabel,fontSize:13}}>Pérdida mínima (%)</div><div style={styles.inputShell}><input type="number" max={-1} value={chatSimLossMinPct} onChange={e=>setChatSimLossMinPct(e.target.value)} style={{...styles.inputInside,paddingLeft:16}} /></div><div style={{...styles.fieldHelp,fontSize:12,fontSize:12}}>Ej: -15</div></div>
          <div><div style={{...styles.fieldLabel,fontSize:13}}>Pérdida máxima (%)</div><div style={styles.inputShell}><input type="number" max={-1} value={chatSimLossMaxPct} onChange={e=>setChatSimLossMaxPct(e.target.value)} style={{...styles.inputInside,paddingLeft:16}} /></div><div style={{...styles.fieldHelp,fontSize:12,fontSize:12}}>Ej: -5. Puede salir “Me sacó con -10%”.</div></div>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'minmax(390px,.82fr) minmax(520px,1.18fr)',gap:14,marginTop:16,alignItems:'stretch'}}>
          <div style={{padding:14,borderRadius:14,border:'1px solid rgba(52,211,153,.20)',background:'rgba(3,18,29,.58)',height:'100%',boxSizing:'border-box'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'minmax(100px,.7fr) 88px 88px minmax(190px,1.3fr) 102px',gap:8}}>
              ...
              <div style={styles.inputShell}><select value={chatSimStrategyId} onChange={e=>setChatSimStrategyId(e.target.value)} style={{...styles.inputInside,paddingLeft:12,appearance:'auto',colorScheme:'dark',background:'#0b1d33',color:'#fff'}}><option value="" style={{background:'#0b1d33',color:'#fff'}}>Seleccionar estrategia</option>{tradeStrategies.filter(s=>s.active).map(s=><option key={s.id} value={s.id} style={{background:'#0b1d33',color:'#fff'}}>{s.name}</option>)}</select></div>
          ...
          <div style={{padding:14,borderRadius:14,border:'1px solid rgba(96,165,250,.20)',background:'rgba(3,18,29,.58)',height:'100%',boxSizing:'border-box'}}>
          ...
          <div style={{gridColumn:'1 / -1',display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:14}}>
            {[
              ...
              <div style={{...styles.fieldLabel,fontSize:15}}>{bank.title}</div>
        ...
        {chatSimulationNotice?<div style={{marginTop:13,padding:'10px 12px',borderRadius:10,border:'1px solid rgba(52,211,153,.25)',background:'rgba(5,150,105,.10)',color:'#d1fae5',fontSize:14,fontWeight:750}}>{chatSimulationNotice}</div>:null}
      </div>
